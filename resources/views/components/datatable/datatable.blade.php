...
<div class="space-y-6" x-data="{
    ...
    <div class="rounded-md border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
                    ...
                    @else
                        @if (isset($filters))
                            @foreach ($filters as $filter)
                                <div class="flex items-center justify-center relative text-xs" x-data="{ open: false }">
                                    <button @click="open = !open"
                                        class="btn-default flex items-center justify-center gap-2 text-xs"
                                        type="button">
                                    ...
                                    <div x-show="open" @click.outside="open = false" x-transition
                                        class="absolute top-10 right-0 mt-2 w-56 rounded-md shadow bg-white dark:bg-gray-700 z-10 p-2 overflow-y-auto max-h-80">
                                        <ul class="space-y-1">
                                            <li class="cursor-pointer text-xs text-gray-700 dark:text-white hover:bg-gray-200 dark:hover:bg-gray-600 px-2 py-1 rounded"
                                                @if ($enableLivewire) wire:click="$set('{{ $filter['id'] }}', ''); $dispatch('resetPage')" @endif
                                                @click="open = false">
                                                {{ $filter['allLabel'] }}
                                            </li>
                                            @foreach ($filter['options'] as $key => $value)
                                                <li class="cursor-pointer text-xs text-gray-700 dark:text-white hover:bg-gray-200 dark:hover:bg-gray-600 px-2 py-1 rounded {{ $filter['selected'] == $key ? 'bg-gray-200 dark:bg-gray-600 font-bold' : '' }}"
        ...
        <div class="flex items-center gap-2 justify-end m-2">
                        ...
                        <div x-show="bulkDeleteModalOpen"
                            ...
                            <div
                                ...
                                @if ($this->getBulkDeleteAction()['url'])
                                    <form id="bulk-delete-form" action="{{ $this->getBulkDeleteAction()['url'] }}"
                                        method="POST">
                ...
                <tbody>
                    @forelse ($data as $item)
                        <tr class="{{ $loop->index + 1 != count($data) ? 'table-tr' : '' }}">
                            ...
                            @foreach ($headers ?? [] as $header)
                                <td class="table-td text-sm {{ isset($header['align']) ? 'text-' . $header['align'] : '' }}">
                                    @php
