<x-layouts.backend-layout :breadcrumbs="$breadcrumbs">
    ...
    <div class="bg-white p-6 rounded-md shadow-md mb-6 dark:bg-gray-800">
        ...
        @if ($selectedLang !== 'en' || ($selectedLang === 'en' && $selectedGroup !== 'json'))
            <form action="{{ route('admin.translations.update') }}" method="POST" data-prevent-unsaved-changes>
                @csrf
                <input type="hidden" name="lang" value="{{ $selectedLang }}">
                <input type="hidden" name="group" value="{{ $selectedGroup }}">
                ...
                <div class="table-responsive">
                    <table
                        class="table min-w-full border divide-y divide-gray-200 dark:divide-gray-700 dark:border-gray-700">
                        <thead class="table-thead">
                            <tr>
                                ...
                                <th scope="col" class="table-thead-th table-thead-th-last">
                                    {{ $languages[$selectedLang]['name'] ?? ucfirst($selectedLang) }}
                                    {{ __('Translation') }}
                                </th>
                            </tr>
                        </thead>
                        <tbody class="bg-white divide-y divide-gray-200 dark:bg-gray-900 dark:divide-gray-700">
                            @foreach ($enTranslations->items() as $key => $value)
                                @if (!is_array($value))
                                    <tr
                                        class="{{ !isset($translations[$key]) ? 'bg-yellow-50 dark:bg-yellow-900/20' : '' }}">
                                ...
                                @else
                                    ...
                                    @foreach ($value as $nestedKey => $nestedValue)
                                        @if (!is_array($nestedValue))
                                            <tr
                                                class="{{ !isset($translations[$key][$nestedKey]) ? 'bg-yellow-50 dark:bg-yellow-900/20' : '' }}">
                                        ...
                                        @else
                                            <tr class="bg-gray-50 dark:bg-gray-700">
                                                <td colspan="3"
                                                    class="table-td font-medium text-gray-700 dark:text-white pl-12">
                                                    <strong>{{ $key }}.{{ $nestedKey }}</strong>
                                                </td>
                                            </tr>
                                            @foreach ($nestedValue as $deepKey => $deepValue)
                                                <tr
                                                    class="{{ !isset($translations[$key][$nestedKey][$deepKey]) ? 'bg-yellow-50 dark:bg-yellow-900/20' : '' }}">
                                                    <td
                                                        class="table-td font-medium text-gray-700 dark:text-white pl-16">
                                                        {{ $deepKey }}
                                                    </td>
                                                    <td class="table-td text-gray-500 dark:text-gray-300">
                                                        {{ $deepValue }}
                    ...
                    <div class="mt-6">
                        {{ $enTranslations->links('pagination::tailwind') }}
                    </div>
                </div>
