    ...
    <div x-show="uploadModalOpen"
        ...
        <div class="flex items-center justify-between border-b border-gray-100 px-6 py-4 dark:border-gray-800 sticky top-0 bg-white dark:bg-gray-700 z-10">
            <h3 class="font-semibold tracking-wide text-gray-700 dark:text-white">
                {{ __('Upload Document to Repository') }}
            </h3>
        ...
        <div class="px-6 pb-6">
            <form id="upload-form" enctype="multipart/form-data">
                ...
                <div class="mb-6">
                    <h4 class="text-lg font-medium text-gray-700 dark:text-white mb-4">{{ __('Document Information') }}</h4>
                    
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                        <!-- Title -->
                        <div class="col-span-2">
                            <label for="document_title" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                                {{ __('Document Title') }} *
                            </label>
                            <input type="text" 
                                   id="document_title" 
                        ...
                        <!-- Author -->
                        <div>
                            <label for="document_author" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                                {{ __('Author') }} *
                            </label>
                            <input type="text" 
                                   id="document_author" 
                    ...
                    <!-- Abstract -->
                    <div class="mb-4">
                        <label for="document_abstract" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                            {{ __('Abstract / Summary') }} *
                        </label>
                        <textarea id="document_abstract" 
                                  name="abstract" 
                                  required
                                  rows="4"
                                  class="w-full rounded-md border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 px-3 py-2 text-sm placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent dark:text-white"
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                        ...
                        <!-- Category -->
                        <div>
                            <label for="document_category" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                                {{ __('Category') }} *
                            </label>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                        <!-- Version -->
                        <div>
                            <label for="document_version" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                                {{ __('Version') }}
                            </label>
                            <input type="text" 
                                   id="document_version" 
                ...
                <div class="mb-6">
                    ...
                    <div class="border-2 border-dashed border-gray-300 dark:border-gray-600 rounded-lg p-8 text-center transition-colors cursor-pointer hover:border-primary hover:bg-primary-50 dark:hover:bg-primary-900/20"
                        ...
                        <input type="file" 
                               ...
                               @else
                               accept=".pdf,.doc,.docx,.xls,.xlsx,.ppt,.pptx,.txt,.csv,.rtf,.odt,.ods,.odp,application/*"
                               @endif
                               class="hidden">
                ...
                <!-- Tags Section -->
                <div class="mb-6">
                    <label for="document_tags" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                        {{ __('Tags') }}
                    </label>
                    <input type="text" 
                           id="document_tags" 
...
document.getElementById('file-input').addEventListener('change', function(e) {
    ...
    files.forEach((file, index) => {
        ...
        if (isDemoMode && !isFileAllowedInDemo(file.type)) {
            errors.push(`{{ __('File ":name" is not allowed in demo mode. Only document files are permitted.', ['name' => '']) }}${file.name}"`);
            return;
        }
...
function uploadDocument() {
    ...
    fetch('{{ route("admin.document.store") }}', {
        method: 'POST',
        body: formData
    })
    .then(response => {
    ...
    .then(data => {
        if (data.success) {
            if (window.showToast) {
                window.showToast('success', '{{ __('Success') }}', data.message || '{{ __('Document uploaded successfully') }}');
            }
