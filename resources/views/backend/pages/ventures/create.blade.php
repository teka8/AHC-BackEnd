<x-layouts.backend-layout :breadcrumbs="$breadcrumbs">
    ...
    <div class="bg-white dark:bg-gray-800 shadow rounded-lg p-6">
        <h2 class="text-2xl font-bold text-gray-900 dark:text-white mb-6">Create New Venture</h2>

        <form method="POST" action="{{ route('admin.ventures.store') }}" enctype="multipart/form-data">
            @csrf
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <select name="focus_area" id="focus_area" required
                                ...
                                <option value="mental-health" {{ old('focus_area') == 'mental-health' ? 'selected' : '' }}>Mental Health</option>
                                <option value="telemedicine" {{ old('focus_area') == 'telemedicine' ? 'selected' : '' }}>Telemedicine</option>
                                <option value="pharmaceuticals" {{ old('focus_area') == 'pharmaceuticals' ? 'selected' : '' }}>Pharmaceuticals</option>
                                <option value="biotech" {{ old('focus_area') == 'biotech' ? 'selected' : '' }}>Biotech</option>
                                <option value="medtech" {{ old('focus_area') == 'medtech' ? 'selected' : '' }}>MedTech</option>
                                <option value="diagnostics" {{ old('focus_area') == 'diagnostics' ? 'selected' : '' }}>Diagnostics</option>
                                <option value="health-tech" {{ old('focus_area') == 'health-tech' ? 'selected' : '' }}>Health Tech</option>
                                <option value="other" {{ old('focus_area') == 'other' ? 'selected' : '' }}>Other</option>
                            </select>
                            @error('focus_area')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        ...
                        <div>
                            ...
                            <select name="stage" id="stage" required
                                ...
                                <option value="early-stage" {{ old('stage') == 'early-stage' ? 'selected' : '' }}>Early Stage</option>
                                <option value="growth" {{ old('stage') == 'growth' ? 'selected' : '' }}>Growth</option>
                                <option value="scale" {{ old('stage') == 'scale' ? 'selected' : '' }}>Scale</option>
                            </select>
                            @error('stage')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div class="md:col-span-2">
                            ...
                            <textarea name="founders" id="founders" rows="2" required
                                      class="form-textarea w-full @error('founders') border-red-500 @enderror">{{ old('founders') }}</textarea>
                            @error('founders')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <input type="number" name="team_size" id="team_size" value="{{ old('team_size') }}" min="1"
                ...
                <div class="border-b border-gray-200 dark:border-gray-700 pb-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                        <div>
                            ...
                            <input type="number" name="funding_raised" id="funding_raised" value="{{ old('funding_raised') }}" 
                                   min="0" step="0.01"
                        ...
                        <div>
                            ...
                            <input type="number" name="patients_impacted" id="patients_impacted" value="{{ old('patients_impacted') }}" 
                                   min="0"
                        ...
                        <div>
                            ...
                            <input type="number" name="countries_reached" id="countries_reached" value="{{ old('countries_reached') }}" 
                                   min="0"
