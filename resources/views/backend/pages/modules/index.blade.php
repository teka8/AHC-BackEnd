<x-layouts.backend-layout :breadcrumbs="$breadcrumbs">
    <div x-data="{ showUploadArea: false }" x-init="showUploadArea = {{ count($modules) > 0 ? 'false' : 'true' }}" x-cloak x-on:keydown.escape.window="showUploadArea = false"
        x-on:click.away="showUploadArea = false">
        <x-slot name="breadcrumbsData">
            <x-breadcrumbs :breadcrumbs="$breadcrumbs">
                <x-slot name="title_after">
                    @if (count($modules) > 0)
                        <button @click="showUploadArea = !showUploadArea" class="ml-4 btn-primary btn-upload-module">
                        ...
                        <x-popover position="bottom" width="w-[300px]">
                            <x-slot name="trigger">
                                <iconify-icon icon="lucide:info" class="text-lg ml-3"
                                    title="{{ __('Module Requirements') }}"></iconify-icon>
                            </x-slot>

                            <div class="w-[300px] p-4 font-normal">
                                <h3 class="font-medium text-gray-700 dark:text-white mb-2">
                                    {{ __('Module Requirements') }}</h3>
                                ...
                                <ul class="list-disc pl-5 space-y-1 text-sm">
                                    ...
                                    <li>
                                        ...
                                        <a href="https://ahcdashboard.com/docs/how-to-create-a-module-in-ahc-dashboard/"
        ...
        @if (!empty($modules))
            <div x-show="showUploadArea"
                class="mb-6 p-6 border-2 border-dashed border-gray-300 rounded-md bg-gray-50 dark:bg-gray-800 dark:border-gray-600"
                @dragover.prevent
                ...
                <p class="text-center text-gray-600 dark:text-gray-300">
                    ...
                    <button @click="$refs.uploadModule.click()" class="text-primary underline hover:text-primary">
                        {{ __('browse') }}
                    </button>
                    {{ __('to select a file.') }}
                </p>
                <form action="{{ route('admin.modules.store') }}" method="POST" enctype="multipart/form-data"
                    class="hidden">
                    @csrf
                    <input type="file" name="module" accept=".zip" x-ref="uploadModule"
            ...
            @if (empty($modules))
                <div class="flex flex-col items-center justify-center h-64 bg-gray-100 dark:bg-gray-800 rounded-md border-2 border-dashed border-gray-300"
                    ...
                    <p class="mt-4 text-gray-600 dark:text-gray-300">
                        {{ __('Drag and drop your module file here, or') }}</p>
                    <button @click="$refs.uploadModule.click()" class="mt-4 btn-primary">
                        <iconify-icon icon="lucide:upload-cloud" class="mr-2"></iconify-icon>
                        {{ __('Upload') }}
                    </button>
                    <form action="{{ route('admin.modules.store') }}" method="POST" enctype="multipart/form-data"
                        ...
                        <div tabindex="0"
                            ...
                            <div class="flex justify-between" x-data="{ deleteModalOpen: false, errorModalOpen: false, errorMessage: '', dropdownOpen: false }">
                                <div class="py-3">
                                    <h2>
                                        <i
                                            class="bi {{ $module->icon }} text-3xl text-gray-500 dark:text-gray-300"></i>
                                    </h2>
                                ...
                                <div class="relative">
                                    <button @click="dropdownOpen = !dropdownOpen"
                                        class="inline-flex items-center h-9 p-2 text-sm font-medium text-center text-gray-700 bg-white rounded-md hover:bg-gray-100 focus:ring-4 focus:outline-none dark:text-white focus:ring-gray-50 dark:bg-gray-800 dark:hover:bg-gray-700 dark:focus:ring-gray-600"
                                        type="button">
                                    ...
                                    <div x-show="dropdownOpen" @click.away="dropdownOpen = false"
                                        ...
                                        <ul class="py-2 text-sm text-gray-700 dark:text-gray-200">
                                            <li>
                                                <button @click="deleteModalOpen = true; dropdownOpen = false"
                                                    ...
                                                    <iconify-icon icon="lucide:trash"
                                                        class="mr-2 text-red-500"></iconify-icon>
                                                    {{ __('Delete') }}
                                                </button>
                                            </li>
                                            <li>
                                                <button
                                                    ...
                                                    class="flex items-center w-full px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white text-left">
                                                    <iconify-icon
                                                        icon="{{ $module->status ? 'lucide:toggle-left' : 'lucide:toggle-right' }}"
                                                        class="mr-2 {{ $module->status ? 'text-green-500' : 'text-gray-500' }}"></iconify-icon>
                                ...
                                <x-modals.confirm-delete id="delete-modal-{{ $module->name }}"
                                ...
                                <x-modals.error-message id="error-modal-{{ $module->name }}"
                                    title="{{ __('Operation Failed') }}" modalTrigger="errorModalOpen" />
                            </div>
                            ...
                            <p class="text-sm text-gray-500 dark:text-gray-300">
                                ...
                                @foreach ($module->tags as $tag)
                                    <span
                                        class="inline-block px-2 py-1 text-xs font-medium text-white bg-gray-400 rounded-full mr-1 mb-1">{{ $tag }}</span>
                                @endforeach
                            </p>
                            <div class="mt-4 flex items-center justify-between">
                                <span
                                    class="text-sm font-medium {{ $module->status ? 'text-green-500' : 'text-red-500' }}">
                        ...
                        if (data.success) {
                            ...
                            if (window.showToast) {
                                window.showToast('success', '{{ __('Success') }}', data.message ||
                                    '{{ __('Module status updated successfully') }}');
                            }
                            ...
                            if (moduleElement && Alpine) {
                                ...
                                component.errorMessage = data.message ||
