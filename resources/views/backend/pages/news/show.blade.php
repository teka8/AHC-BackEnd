    ...
    <div class="space-y-6">
        <div class="rounded-md border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
            <div class="px-5 py-4 sm:px-6 sm:py-5 flex justify-between items-center border-b border-gray-100 dark:border-gray-800">
                <h3 class="text-base font-medium text-gray-700 dark:text-white/90">{{ __('News Details') }}</h3>
                <div class="flex gap-2">
                    @if (auth()->user()->can('post.edit'))
                        <a href="{{ route('admin.news.edit', [$postType, $post->id]) }}" class="btn-primary">
                            <iconify-icon icon="lucide:pencil" class="mr-2"></iconify-icon>
                            {{ __('Edit') }}
                        </a>
                    @endif
                    <a href="{{ route('admin.news.index', $postType) }}" class="btn-default">
            ...
            <div class="px-5 py-4 sm:px-6 sm:py-5">
                ...
                <div class="mb-6 flex flex-wrap gap-4 text-sm text-gray-600 dark:text-gray-300">
                    <div class="flex items-center">
                        ...
                        <span class="text-sm text-gray-500 dark:text-gray-400">{{ __('Author:') }} {{ $post->author ?? $post->user->full_name }}</span>
                    </div>
                    @if($post->news_source)
                    <div class="flex items-center">
                        <iconify-icon icon="lucide:newspaper" class="mr-1"></iconify-icon>
                        {{ __('Source:') }} {{ $post->news_source }}
                    </div>
                    @endif
                    @if($post->priority)
                    <div class="flex items-center">
                        <iconify-icon icon="lucide:flag" class="mr-1"></iconify-icon>
                        {{ __('Priority:') }} 
                        <span class="ml-1 px-2 py-1 text-xs rounded-full 
                            @if($post->priority === 'urgent') bg-red-100 text-red-800
                            @elseif($post->priority === 'high') bg-orange-100 text-orange-800
                            @elseif($post->priority === 'normal') bg-blue-100 text-blue-800
