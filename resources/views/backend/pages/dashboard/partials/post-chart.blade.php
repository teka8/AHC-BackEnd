<div class="col-span-12">
    <div class="rounded-md border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-700 dark:bg-gray-800 sm:p-6">
        <div class="mb-4 flex items-center justify-between">
            <div>
                <h3 class="text-lg font-semibold text-gray-700 dark:text-white">{{ __('News Activity') }}</h3>
                <p class="text-sm text-gray-500 dark:text-gray-300">{{ __('News status over time') }}</p>
            </div>
        </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const options = {
        series: [
            ...
            {
                name: '{{ __("Reviewed") }}',
                data: postData.reviewed || []
            },
            {
                name: '{{ __("Approved") }}',
                data: postData.approved || []
            },
            {
                name: '{{ __("Published") }}',
        ...
        xaxis: {
            ...
            labels: {
                style: { colors: '#64748b', fontFamily: 'var(--font-sans)' }
            }
        },
        yaxis: {
            title: {
                text: '{{ __("Number of News") }}',
        ...
        fill: {
            opacity: 1,
            colors: ['#6366f1', '#f97316', '#10b981', '#facc15', '#ef4444']
        },
        tooltip: {
            y: {
                formatter: function (val) { return val + " {{ __('news') }}"; }
            },
